    ...
    public List<Phone> download(String url) throws IOException {
        ...
        for (int i = 0; i < all.size(); i++) {
            try {
                ...
                String tempBuiltInMemory = all.get(i).getElementsByClass("features").select("ul").select("li").get(2).text();
                tempBuiltInMemory = tempBuiltInMemory.replaceAll(",", ".");
                if (tempBuiltInMemory.contains("+")) {
                    int index = tempBuiltInMemory.indexOf("+");
                    if (index != -1) {
                        tempBuiltInMemory = tempBuiltInMemory.substring(0, index);
                    }
                }
                phone.setBuiltInMemory(findNumberInString(tempBuiltInMemory));
